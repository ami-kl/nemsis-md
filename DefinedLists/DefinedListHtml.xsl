...
<xsl:stylesheet version="1.0" xmlns:n="http://www.nemsis.org" xmlns="http://www.nemsis.org" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:output method="html" encoding="UTF-8"/>

  <xsl:variable name="XSLT" select="document('')"/>
  <xsl:variable name="DL" select="/n:DefinedList"/>

  <xsl:key name="KeyNemsisTypes" match="n:type" use="@name"/>
  <xsl:key name="KeyNemsisElements" match="n:element" use="@name"/>
  <xsl:key name="KeyCodeLookup" match="xs:restriction/xs:enumeration" use="@value"/>
  ...
  <xsl:variable name="listName">
    <xsl:choose>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eInjury.01'">Cause of Injury</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eHistory.07'">Environmental/Food Allergy</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eSituation.11'">Impression</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eScene.09'">Incident Location Type</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eHistory.08'">Medical/Surgical History</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eMedications.03'">Medication</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eHistory.06'">Medication Allergy</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eSituation.17'">Patient Activity</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eProcedures.03'">Procedure</xsl:when>
      <xsl:when test="$DL/n:Header/n:NemsisElements/n:NemsisElement = 'eSituation.09'">Symptom</xsl:when>
    </xsl:choose>
  </xsl:variable>
  <xsl:variable name="CodesCount">
    <xsl:value-of select="count($DL/n:Codes/n:Code)"/>
  </xsl:variable>
  ...
  <xsl:template match="/n:DefinedList">
    <html xmlns="">
      ...
      <head>
        <title>
          <xsl:value-of select="$listName"/> | NEMSIS v3 Defined List
        </title>
        ...
        <style type="text/css">
          ...
          tbody:nth-child(even) {
            ...
            border-bottom: 1ex solid #f2faff;
          }
        ...
        <script>
          <![CDATA[
          // sortTable: Sort table by specified column
          function sortTable(tableId, colNum, reverse) {
            var table = document.getElementById(tableId),
                tb = Array.prototype.slice.call(table.tBodies, 0),
                i;
            reverse = -((+reverse) || -1);
            tb = tb.sort(function (a, b) {
                return reverse
                    * (a.rows[0].cells[colNum].textContent.trim()
                        .localeCompare(b.rows[0].cells[colNum].textContent.trim())
                       );
            });
            for(i = 0; i < tb.length; ++i) table.appendChild(tb[i]);
          }
          ]]>
        </script>
      ...
      <body>
        ...
        <div id="header">
          ...
          <h1>
            NEMSIS v3 Defined List<br/>
            <strong><xsl:value-of select="$listName"/></strong><br/>
            NEMSIS Version <xsl:value-of select="$version"/><br/>
            Version Date: <xsl:value-of select="concat(substring(@date, 6, 2), '/', substring(@date, 9, 2), '/', substring(@date, 1, 4))"/><br/>
          </h1>

          <h2>Source Vocabulary(s)</h2>
          <ul>
            <xsl:for-each select="$DL/n:Header/n:SourceVocabularies/n:SourceVocabulary">
              <li><a href="{@url}" target="_new"><xsl:value-of select="."/></a></li>
            </xsl:for-each>
          </ul>

          <h2>NEMSIS Type(s)</h2>
          <ul>
            <xsl:for-each select="$DL/n:Header/n:NemsisTypes/n:NemsisType">
              <li>
                <xsl:variable name="nemsisType" select="."/>
                <xsl:for-each select="$XSLT">
                  <xsl:choose>
                    <xsl:when test="key('KeyNemsisTypes', $nemsisType)">
                      <xsl:variable name="dataDictionaryUrl" select="concat($urlBase, 'DataDictionary/APIs/EMSDataSetAPI/', key('KeyNemsisTypes', $nemsisType), '_v3_xsd.html#', $nemsisType)"/>
                      <a href="{$dataDictionaryUrl}" target="_new">
                        <xsl:value-of select="$nemsisType"/>
                      </a>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="$nemsisType"/>
          ...
          <h2>NEMSIS Element(s)</h2>
          <ul>
            <xsl:for-each select="$DL/n:Header/n:NemsisElements/n:NemsisElement">
              <li>
                <xsl:variable name="nemsisElement" select="."/>
                <xsl:variable name="dataDictionaryUrl" select="concat($urlBase, 'DataDictionary/PDFHTML/', $urlDDPath, '/sections/elements/', $nemsisElement, '.xml')"/>
                <xsl:for-each select="$XSLT">
                  <xsl:choose>
                    <xsl:when test="key('KeyNemsisElements', $nemsisElement)">
                      <a href="{$dataDictionaryUrl}" target="_new">
                        <xsl:value-of select="$nemsisElement"/>
                      </a>
                      -
                      <xsl:value-of select="key('KeyNemsisElements', $nemsisElement)"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of select="$nemsisElement"/>
                    </xsl:otherwise>
          ...
          <p class="footer">Items on list: <xsl:value-of select="$CodesCount"/></p>
        ...
        <div class="page" id="root">
          <table id="CodesTable">
            <thead>
              <tr class="state">
                <th class="value sortable" onclick="sortTable('CodesTable', 0)" title="Sort by Value">Value</th>
                <th class="value sortable" onclick="sortTable('CodesTable', 1)" title="Sort by Code Type">Code Type</th>
                <th class="value sortable" onclick="sortTable('CodesTable', 2)" title="Sort by Category">Category</th>
                <th class="value sortable" onclick="sortTable('CodesTable', 3)" title="Sort by Source Label">Source Label</th>
                <th class="value sortable" onclick="sortTable('CodesTable', 4)" title="Sort by Suggested Label">Suggested Label</th>
                <th class="value sortable" onclick="sortTable('CodesTable', 5)" title="Sort by Date">Date</th>
              </tr>
            </thead>
            <xsl:for-each select="n:Codes/n:Code">
              <tbody>
                <tr>
                  <td class="value">
                  ...
                  <td class="label">
                    <xsl:choose>
                      <xsl:when test="n:Value/@CodeType = '9924001'">ICD-10</xsl:when>
                      <xsl:when test="n:Value/@CodeType = '9924003'">RxNorm</xsl:when>
                      <xsl:when test="n:Value/@CodeType = '9924005'">SNOMED-CT</xsl:when>
                    </xsl:choose>
                  </td>
                  <td class="label">
                    <xsl:value-of select="n:Category" />
                  </td>
                  <td class="label">
                    <xsl:value-of select="n:SourceLabel" />
                  </td>
                  <td class="label">
                    <xsl:value-of select="n:SuggestedLabel" />
                  </td>
                  <td class="value">
  ...
  <xsl:template name="lookup">
    <xsl:param name="Code"/>
    <xsl:for-each select="$XSLT">
      <xsl:value-of select="key('KeyCodeLookup', $Code)[not(../../@version) or ../../@version=$version]/xs:annotation/xs:documentation" />
    </xsl:for-each>
  </xsl:template>

  <!--  Container for lookup lists -->
  <lookup>

    <NemsisTypes>
      <type name="icd10Code">commonTypes</type>
      <type name="icd10Activity">eSituation</type>
      <type name="icd10CodeInjury">commonTypes</type>
      <type name="icd10MedSurge">commonTypes</type>
      <type name="snomed">commonTypes</type>
      <type name="MedicationAllergies">eHistory</type>
      <type name="Medication">commonTypes</type>
      <type name="IncidentLocationType">eScene</type>
      <type name="ProvidersImpression">eSituation</type>
      <type name="AssociatedSymptoms">eSituation</type>
    </NemsisTypes>

    <NemsisElements version="3.5">
